...
def CarbonCopy(host, port, signee, signed):
    ...
    try:
        ...
        if (platform == "win32"):
            ...
            print(subprocess.check_output("signtool.exe sign /a /v /f " + PFXFILE + " /d \"MozDef Corp\" /tr \"http://sha256timestamp.ws.symantec.com/sha256/timestamp\" /td SHA256 /fd SHA256 /as " + signed, shell=True).decode())
        else:
